...
import { kitByPlatform, listMediaKits } from "@/lib/media-kit/queries";
import {
  PLATFORMS,
  PLATFORM_LABELS,
  canVerifyAudience,
 ...
 *
 * The page holds the product's central claim together: every number a sponsor
 * eventually sees is either pulled from a platform API or carries a
 * self-reported tag, and the two are never rendered without saying which is
 * which (§7.2). What a creator types lands in declared_top_countries; the
 * verified column is unreachable from the browser's identity by RLS, not by
 * this page choosing not to write it.
 */
